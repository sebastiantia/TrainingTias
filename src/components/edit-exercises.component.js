...
class EditExercises extends Component {
  ...
  async componentDidMount() {
    ...
    const response = await axios.get('http://localhost:4000/exercises/'+this.props.match.params.id)
    try {
      this.setState({
  ...
  async onSubmit(e) {
    ...
    try {
      const post = await axios.post(
        "http://localhost:4000/exercises/udpate/"+this.props.match.params.id,
        exercise
      );
